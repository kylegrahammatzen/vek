...
func Find(x []float32, a float32) int {
	if functions.UseAVX2 {
		idx := functions.Find_AVX2_F32(x, a)
		if idx == len(x) {
			return -1
		}
		return idx
...
func Range(a, b float32) []float32 {
	dst := make([]float32, int(math32.Max(0, math32.Ceil(b-a))))
...
func Range_Into(dst []float32, a, b float32) []float32 {
	n := int(math32.Max(0, math32.Ceil(b-a)))
	if cap(dst) < n {
		panic("destination slice not large enough to hold result")
	}
	if functions.UseAVX2 {
...
// FromFloat64 creates a slice of floats from a slice of 64-bit floats. Standard conversion
// rules apply.
func FromFloat64(x []float64) []float32 {
	dst := make([]float32, len(x))
	return FromFloat64_Into(dst, x)
}

// FromFloat64_Into creates a slice of floats from a slice of 32-bit floats. Standard conversion
// rules apply. The result is stored in the destination slice.
...
// ToFloat64 converts a slice of floats to a slice of 32-bit floats. Standard conversion rules
// apply.
func ToFloat64(x []float32) []float64 {
	dst := make([]float64, len(x))
	return ToFloat64_Into(dst, x)
}

// ToFloat64_Into converts a slice of floats to a slice of 64-bit floats. Standard conversion
// rules apply. The result is stored in the destination slice.
